...
.firstPass <- function(yval, design, ignoreNegative, epsilon=1e-4) {
  ...
  # HERE IS THE PROBLEM!  Quantiles forces a double trim in conjunction with calcLogitz and epsilon above 
  # artificially shrinking alpha and beta
  # Try to match 0.12
  ...
  dat <- unlist(lapply(series, function(ser, dsn, ld) {
    items <- dsn@layout$Series == ser   # get the items in this diluton series
    steps <- dsn@layout$Steps[items]    # get the log2 steps for this series
  ...
  minslope <- 0.001  # force a non zero slope for starting estimate, and filter out zero slopes from blank sample
  dat <- dat[dat > minslope]
  
  # use mean, not median.  We may have many blank samples on a slide in which case median slope will be 0
  gamma <- mean(dat, trim = 0.01)
  gamma <- max(gamma, minslope) # force a non zero slope for starting estimate
  ...
  names(passer) <- series
  for (this in series) {
    items <- design@layout$Series == this
    passer[this] <- median(lindata[items]/gamma-design@layout$Steps[items], na.rm=TRUE) 
  }
  
  list(lBot=lBot, lTop=lTop, gamma=gamma, passer=passer)
}
...
.loess.line <- function(xval, yval, col = "red", span = 2/3, xform=function(x) x) {
...
setMethod("plot", "RPPAFit",
  ...
  if (type == "cloud" | type == "series") {
	 ...
	 if(!hasArg(sub)) {
	 	autosub <- paste('(Conc > -5) Trimmed Mean R^2 =', format(mean(x@ss.ratio[x@concentrations > -5], trim=0.1), digits=3), 
	 	  ', Min / Max Valid Conc. =', round(min.conc,2),
	      '/', round(max.conc,2))
    ...
    for (this in series) {
      ...
      points(x@concentrations[this], x@intensities[this], pch=16, col='red')
      items <- x@design@layout$Series == this
    ...
    lines(sort(xval), sort(yval), lwd=2)
      ...
      xfit <- xfit[-1]
      yfit <- yfit[-1]
      plot(xplot, yplot, xlab = "(Step[n+1]+Step[n])/2", ylab = "Step[n+1] - Step[n]" )
      ...
      legend("bottomleft", c("loess", "model"), col = c("red", model.color), lty = 1)
  } else if (type == "resid") {
	  # show a plot of the residuals v.s. estimated concentration to check for heteroscedasticity
	  r <- resid(x)
	  
	  # fit a model of abs(residual) v.s. estimated conc. to do a rough check for increasing heteroscedasticity
	  l <- lm(abs(r) ~ xval)
	  s <- summary(l)
      vals <- s$coefficients["xval", c("Estimate", "Pr(>|t|)")]
	  subt <- paste("abs(Residual) Slope:", round(vals[1],2), ", p-value = ", round(vals[2],3)) 
...
RPPAFitFromParams <- function(rppa, design, fitparams) {
    ...
    RPPAFit(rppa, design, fitparams@measure, fitparams@xform, 
      fitparams@method, fitparams@trim, fitparams@ci, fitparams@ignoreNegative, 
...
RPPAFit <- function(rppa,
  ...
  for (pass in 1:2) {  
	  # do a two pass estimation. first using rough conc. estimates, then using better ones
	  if (pass == 1) {
	  ...
	  for (this in series) {
		...
		fs <- fitSeries(fc, diln = steps[items], intensity = yval[items], est.conc = passer[this], method = method, silent = silent, trace = trace)
  ...
  if(ci) { # compute confidence intervals for the estimates
...
getConfidenceInterval <- function(result, alpha=0.10, nSim=50) { 
  ...
  for (this in series) {
    ...
    sim <- rep(NA, nSim)
    for (j in 1:nSim) { 
      resid.boot <- rnorm(sum(items), 0, sigma[items]) # sample the residuals
      #add resid.boot to y_hat;  refit; 
      ysim <- yhat + resid.boot
      
      fs <- fitSeries(result@model, diln = steps[items], intensity = ysim, est.conc = xhat[1], method = method, silent = silent, trace = F)
	  sim[j] <- fs$est.conc      
    }
