                  ...
                  ...)
{
    ...
    oform <- formula
    formula <- as.formula(substitute(~(LHS-RHS) * w,
                                     list(LHS=formula[[2]],
                                          RHS=formula[[3]])))
    ##- robust loop (IWLS)
    converged <- FALSE
    ...
    if (trace && !converged && !method.exit) {
        status <- paste("failed to converge in", maxit, "steps")
        if (trace) {
            warning(status)
        }
    }
