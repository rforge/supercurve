%\VignetteIndexEntry{RPPA Primer, ver 0.9999}
%\VignetteKeywords{Preprocessing, Lysate Arrays}
%\VignetteDepends{SuperCurve}
...
$$
where $g(x) = e^x/(1+e^x)$. The shape paramters of the 
...
\begin{itemize}
  \item \Robject{RPPA}, representing the quantification of an array,
  \item \Robject{RPPADesign}, specifying where each sample/dilution
        step combination is printed on the array, and
  \item \Robject{RPPAFit}, the results of fitting the SuperCurve
        model to an \Robject{RPPA}, \Robject{RPPADesign} combination.
\end{itemize}
More details can be found in the documentation for 
\Robject{RPPA}, \Robject{RPPA-class},
\Robject{RPPADesign}, \Robject{RPPADesign-class},
\Robject{RPPAFit}, and \Robject{RPPAFit-class},
...
data are included as the \Robject{rppaCell} data file, 
described as \Robject{rppaCellData}.  
...
\Robject{RPPADesign} object, {\tt design40}, giving the layout of
the array. For each spot, this specifies what sample has 
been printed, and what the dilution level offset of that 
spot is (in $\log_2$ units) relative to the reference 
level we're trying to estimate. Second, we have several
\Robject{RPPA} objects, {\tt akt}, {\tt c.erk2}, and {\tt ctnnb1},
each containing the spot-level quantifications supplied
by the microarray image quantification software. These arrays
...
These also allow us to illustrate how an \Robject{RPPA} object can
...
class(aktTake2);
@
%aktTake2 <- RPPA(file = "rppaCellData/AKT.txt",
%    path = system.file(package = "SuperCurve");
This \Robject{RPPA} object is the same as {\tt akt}. Now, there's
really not that much to an \Robject{RPPA} object:
<<getStarted03>>=
slotNames(aktTake2);
aktTake2@file;
@
...
Now, we've seen what the {\tt file} slot contains, but
what about {\tt data}? As mentioned above, it's simply
a data frame, with named components corresponding for 
the most part to measurements associated with each spot. 
Several of the functions that act on \Robject{RPPA} objects have 
a "measure" argument, and they're simply looking for 
...
names(aktTake2@data)
@
Most of the columns have ready interpretations, but 
the first 6 here are special. An \Robject{RPPA} object must have
{\tt Main.Row}, {\tt Main.Col}, {\tt Sub.Row}, and {\tt Sub.Col} 
to specify the position of the spot, {\tt Sample} to tell 
us what the software thinks was printed, and {\tt Mean.Net}
as a background-corrected measure of spot intensity. 

The constructor function is tuned for MicroVigene files
at present. If we have other types of data files, we 
might want to assemble an \Robject{RPPA} object more directly.
<<getStarted05>>=
aktRead <- paste(superHome,aktHome,sep="/");
aktTemp <- read.table(file = aktRead,
                      header = T, sep="\t", skip = 4, fill = T);
names(aktTemp)[5] <- "Sample";
names(aktTemp)[6] <- "Mean.Net"; 
aktTake3 <- new("RPPA", data = aktTemp, file = aktRead); 
@ 
again, this \Robject{RPPA} object is just the same as
the others that we have assembled for AKT. 

Given an \Robject{RPPA}, one of the first things we
...
at Main.Row 3, Main.Col 6 (try calling \Rfunction{image}
...
detach("akt@data");
@
Similar checks of the background for the other \Robject{RPPA}
...
@ 
The \Robject{RPPADesign} object has 4 slots: \Robject{layout}, 
...
lets us subset the measurments within a sample, if desired. To 
...
The trickiest part of constructing an \Robject{RPPADesign} object 
...
@ 
This version of the constructor extracts some of the 
needed layout information from the \Robject{RPPA} object itself. 
Now, we can construct an \Robject{RPPADesign} using a more
low-level approach, such as reading the Sample information
(and possibly the dilution levels as well) from a separate
file. 
<<getStarted15>>=
superHome <- system.file(package="SuperCurve");
aktHome <- "rppaCellData/Akt.txt";
...
aktTemp <- read.table(file=aktRead,
                      header=T, sep="\t", skip=4, fill=T);
names(aktTemp)[5] <- "Sample";
aktTemp <- aktTemp[, names(aktTemp) %in% 
                   c("Main.Row", "Main.Col", "Sub.Row", "Sub.Col", 
                     "Sample")];
steps <- rep(c(rep(8:5, 2), rep(4:1, 2)), 40) - 4.5;
rep.temp <- factor(paste('Rep', rep(rep(1:2, each=4), 80), sep=''));
...
Given an \Robject{RPPADesign} and an \Robject{RPPA}, we
can use a model to estimate the protein concentrations,
producing an \Robject{RPPAFit}. The fitting function 
requires that we specify the measure to be used, in
addition to the \Robject{RPPA} and \Robject{RPPADesign}. 
<<getStarted20>>=
...
\Robject{RPPADesign}, grouping all spots from a single
sample together. 
<<getStarted28>>=
steps <- rep(c(rep(8:5, 2), rep(4:1, 2)), 40) - 4.5;
rep.temp <- factor(paste('Rep', rep(rep(1:2, each=4), 80), sep=''));
