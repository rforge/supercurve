...
%\VignetteDepends{methods, stats, utils, graphics, grDevices, MASS, cobs}
...
described as \Robject{data?rppaCell}.  
...
<<getStarted02>>=
filename <- "Akt.txt";
directory <- system.file("rppaCellData", package="SuperCurve");
aktTake2 <- RPPA(file=filename, path=directory);
class(aktTake2);
@
%aktTake2 <- RPPA(file="Akt.txt",
...
The generator function is tuned for MicroVigene files
at present. If we have other types of data files, we 
might want to assemble an \Robject{RPPA} object more directly.
<<getStarted05>>=
filename <- "Akt.txt";
aktTemp <- read.delim(file=file.path(directory, filename), skip=4);
names(aktTemp)[5] <- "Sample";
names(aktTemp)[6] <- "Mean.Net";
aktTake3 <- new("RPPA", data=aktTemp, file=filename);
@ 
...
Given an \Robject{RPPA} and an \Robject{RPPADesign}, we
